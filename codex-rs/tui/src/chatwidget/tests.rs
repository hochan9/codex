...
fn test_named_agent(id: &str, name: &str, prompt: &str) -> AgentDefinition {
    AgentDefinition {
        id: id.to_string(),
        name: name.to_string(),
        description: None,
...
async fn make_chatwidget_manual(
    ...
    let mut widget = ChatWidget {
        ...
        command_description_language: crate::slash_command::CommandDescriptionLanguage::English,
        collab_progress_board: crate::collab::ProgressBoard::default(),
        auth_manager,
        models_manager,
        otel_manager,
...
#[tokio::test]
async fn active_named_agent_augments_collaboration_mode_and_overrides_model_settings() {
    let (mut chat, _rx, mut op_rx) = make_chatwidget_manual(Some("gpt-5")).await;
    chat.thread_id = Some(ThreadId::new());
    chat.set_feature_enabled(Feature::CollaborationModes, true);
    let mut agent = test_named_agent(
        "reviewer",
        "Code Reviewer",
        "Always include a regression checklist in the answer.",
    );
    agent.model = Some("gpt-5.2-codex-mini".to_string());
    agent.reasoning_effort = Some(ReasoningEffort::High);
    chat.set_active_named_agent(Some(agent));

    chat.submit_user_message("Review this diff".to_string().into());
    ...
    let Op::UserTurn {
        ...
        panic!("expected Op::UserTurn with collaboration mode");
    };

    assert_eq!(mode.settings.model, "gpt-5.2-codex-mini");
    assert_eq!(mode.settings.reasoning_effort, Some(ReasoningEffort::High));
    let developer_instructions = mode
        .settings
        .developer_instructions
        .expect("expected merged developer instructions");
    assert!(developer_instructions.contains("active_named_agent"));
    assert!(developer_instructions.contains("regression checklist"));
}

#[tokio::test]
async fn named_agent_registry_is_injected_for_delegation() {
    let (mut chat, _rx, mut op_rx) = make_chatwidget_manual(Some("gpt-5")).await;
    chat.thread_id = Some(ThreadId::new());
    chat.set_feature_enabled(Feature::CollaborationModes, true);
    let mut frontend_agent = test_named_agent(
        "frontend",
        "Frontend Specialist",
        "Focus on UI reliability.",
    );
    frontend_agent.description = Some("Own React and styling concerns.".to_string());
    chat.set_named_agents_registry(vec![frontend_agent]);

    chat.submit_user_message("Build a settings screen".to_string().into());
    ...
    let developer_instructions = mode
        .settings
        .developer_instructions
        .expect("expected delegation instructions");
    assert!(developer_instructions.contains("<named_agents_registry>"));
    assert!(developer_instructions.contains("id: frontend"));
    assert!(developer_instructions.contains("Focus on UI reliability."));
    assert!(!developer_instructions.contains("<active_named_agent"));
}

#[tokio::test]
async fn model_display_name_shows_active_named_agent_label() {
    let (mut chat, _rx, _op_rx) = make_chatwidget_manual(Some("gpt-5")).await;
    let agent = test_named_agent("architect", "Architect", "Design first.");
    chat.set_active_named_agent(Some(agent));

    assert_eq!(chat.model_display_name(), "gpt-5 [agent:Architect]");
}

#[tokio::test]
async fn delegation_policy_engine_is_not_prompt_injected_anymore() {
    let (mut chat, _rx, mut op_rx) = make_chatwidget_manual(Some("gpt-5")).await;
    chat.thread_id = Some(ThreadId::new());
    chat.set_feature_enabled(Feature::CollaborationModes, true);
    chat.set_named_agents_registry(vec![
        test_named_agent("frontend", "Frontend", "Focus on UI structure."),
        test_named_agent("backend", "Backend", "Focus on API and persistence."),
    ]);
    let complex_prompt = "Build a React settings UI, update backend API handlers, and tighten CI deploy checks across multiple files.\nStep 1: map affected files.\nStep 2: implement UI+API changes.\nStep 3: add tests and docs.";
    chat.submit_user_message(complex_prompt.to_string().into());
    ...
    let developer_instructions = mode
        .settings
        .developer_instructions
        .expect("expected named agent registry instructions");
    assert!(!developer_instructions.contains("<delegation_policy_engine>"));
}

#[tokio::test]
async fn collab_progress_board_renders_as_fixed_panel() {
    let (mut chat, _rx, _op_rx) = make_chatwidget_manual(Some("gpt-5")).await;
    ...
    assert!(
        collapsed.contains("Sub-agent board"),
        "expected fixed sub-agent board panel in render: {screen}"
    );
    assert!(
        collapsed.contains("queued: 1 workers"),
        "expected queue counters in board panel: {screen}"
    );
}
